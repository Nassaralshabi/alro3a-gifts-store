...
export default function ProductCard({ product: entry }: { product: CatalogProduct }) {
  ...
  return <article className="group overflow-hidden rounded-2xl border border-[#ebe7df] bg-white [contain-intrinsic-size:auto_430px] [content-visibility:auto] transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_30px_-22px_rgba(42,32,54,0.38)]">
    <Link href={`/products/${product.slug}`} className="relative block aspect-[1/1.03] overflow-hidden bg-[#f3edf6]">
      <img src={product.imageUrl ?? FALLBACK_IMAGE} alt={title} width={640} height={660} loading="lazy" decoding="async" fetchPriority="low" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
