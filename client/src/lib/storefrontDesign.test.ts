...
describe("storefront design refresh", () => {
  it("keeps responsive design hooks for the hero, navigation, catalogue filters, and product cards", () => {
    ...
    expect(home).toContain('className="border-b border-[#dbe9eb] bg-[#f6f8f4]"');
    ...
    const catalogSections = source("client/src/components/CatalogSections.tsx");
    expect(catalogSections).toContain('raed-container py-5 sm:py-7');
    expect(catalogSections).toContain('raed-container py-8 sm:py-11');
  });
});
