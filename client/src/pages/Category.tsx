...
export default function Category() {
  ...
  if (!category) return <StoreShell><div className="grid min-h-[60vh] place-items-center px-5 text-center"><div><h1 className="font-display text-3xl">{isArabic ? "لم نجد هذه الخدمة" : "We couldn't find this service"}</h1><Button asChild className="mt-5 rounded-xl bg-[#7953a2]"><Link href="/">{isArabic ? "العودة للرئيسية" : "Back home"}</Link></Button></div></div></StoreShell>;
  const content = isArabic ? category.ar : category.en;
  const Icon = category.icon;

  return <StoreShell><section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:py-16"><Link href="/#services" className="inline-flex items-center gap-2 text-sm font-bold text-[#7953a2]"><ArrowRight className={`h-4 w-4 ${direction === "ltr" ? "rotate-180" : ""}`} />{isArabic ? "كل الخدمات" : "All services"}</Link><div className="mt-7 grid overflow-hidden rounded-[2rem] bg-[#f0eaf5] lg:grid-cols-[1.15fr_.85fr]"><div className="p-8 sm:p-12"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-[#7953a2] text-white"><Icon className="h-6 w-6" /></div><p className="mt-7 text-xs font-black uppercase tracking-[0.2em] text-[#a27ab2]">{content.eyebrow}</p><h1 className="mt-4 max-w-2xl font-display text-4xl leading-tight text-[#24233a] sm:text-5xl">{content.title}</h1><p className="mt-5 max-w-xl text-sm leading-8 text-[#6c6562] sm:text-base">{content.description}</p><Button asChild className="mt-8 h-12 rounded-xl bg-[#7953a2] px-6 hover:bg-[#654287]"><a href={WHATSAPP_URL} target="_blank" rel="noreferrer"><MessageCircleMore />{isArabic ? "اطلبي هذه الخدمة" : "Request this service"}</a></Button></div><div className="bg-[#24233a] p-8 text-white sm:p-12"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#dcb65c]">{isArabic ? "أفكار شائعة" : "POPULAR IDEAS"}</p><div className="mt-7 grid gap-3">{content.ideas.map((idea, index) => <div key={idea} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-4"><span className="grid h-8 w-8 place-items-center rounded-full bg-[#dcb65c] text-xs font-black text-[#24233a]">0{index + 1}</span><span className="font-bold">{idea}</span></div>)}</div></div></div></section></StoreShell>;
}
