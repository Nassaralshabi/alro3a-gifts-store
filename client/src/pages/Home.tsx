...
const PAPER_CUPS_BRANCHES_BANNER = "/manus-storage/alrawhaa-paper-cups-branches_4d0b6559.webp";
...
export default function Home() {
  ...
  return <StoreShell logoUrl={HERO_BRAND_LOGO}>
    ...
    <section className="border-b border-[#e2ecee] bg-white py-2 sm:py-3"><div className="mx-auto w-full max-w-[1440px] px-2 sm:px-3 lg:px-4"><img src={PAPER_CUPS_BRANCHES_BANNER} alt={isArabic ? "بانر أكواب ورقية وفروع مطبعة الروعة" : "Al Rawaa paper cups and branches banner"} width={2048} height={877} loading="eager" decoding="async" className="block w-full rounded-xl border border-[#e5edf0] object-contain shadow-[0_10px_28px_-24px_rgba(16,47,57,.45)] sm:rounded-2xl" /></div></section>
    {isLoading ? <section className="border-b border-[#e2ecee] bg-white"><div className="raed-container py-5 sm:py-6"><div className="h-8 w-52 animate-pulse rounded bg-[#e8f1f2]" /><div className="mt-4 flex gap-3 overflow-hidden">{Array.from({ length: 5 }).map((_, index) => <div key={index} className="h-32 min-w-[125px] animate-pulse rounded-2xl bg-[#edf4f5]" />)}</div></div></section> : <CategoryRail categories={homeCatalog?.categories || []} isArabic={isArabic} />}
