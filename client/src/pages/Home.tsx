...
import { ArrowLeft, ArrowUpLeft, Award, Boxes, Gift, LayoutPanelTop, Package, Paintbrush, Sparkles, Stamp, Tag, Truck, type LucideIcon } from "lucide-react";
...
export default function Home() {
  ...
  const { data: products = [], isLoading } = trpc.store.catalog.products.useQuery(undefined, queryOptions);
  ...
  return <StoreShell>
      ...
      <div className="mx-auto max-w-[1440px] px-4 py-4 sm:px-8">
        <div className="relative grid min-h-[360px] overflow-hidden rounded-2xl bg-[#eee5f3] lg:grid-cols-[1.08fr_.92fr]">
          ...
          <div className="relative order-1 min-h-[280px] p-6 lg:order-2 lg:p-9">
            ...
            <img src={heroImage} alt={isArabic ? "بانر مخصص من مطبعة الروعة" : "Custom banner by Al Rawaa"} width={960} height={720} decoding="async" fetchPriority="high" className="relative h-full w-full rounded-xl object-cover shadow-[0_25px_50px_-28px_rgba(36,35,58,.75)]" />
            <span className="absolute bottom-10 start-10 rounded-xl bg-[#24233a] px-4 py-3 text-white shadow-lg"><span className="block text-[10px] font-bold tracking-[.15em] text-[#dcb65c]">{isArabic ? "الأكثر طلبًا" : "POPULAR PICK"}</span><span className="mt-1 block font-display text-lg">{isArabic ? "لوحات التصوير" : "Photo boards"}</span></span>
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="mx-auto max-w-[1440px] px-4 py-10 [contain-intrinsic-size:auto_720px] [content-visibility:auto] sm:px-8">
      ...
      {isLoading ? <div className="mt-7 grid grid-cols-2 gap-4 lg:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-72 animate-pulse rounded-xl bg-[#f4f0f6]" />)}</div> : products.length ? <div className="mt-7 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">{products.slice(0, 8).map(product => <ProductCard key={product.product.id} product={product} />)}</div> : <div className="mt-7 rounded-2xl border border-dashed border-[#d6c9dd] bg-[#faf8fb] p-8 text-center"><Award className="mx-auto h-7 w-7 text-[#7953a2]" /><p className="mt-3 text-sm text-[#766f69]">{isArabic ? "لا توجد منتجات منشورة بعد. أرسلي فكرتك وسنحوّلها إلى طلب خاص." : "There are no published products yet. Share your idea and we will turn it into a custom request."}</p></div>}
    </section>
