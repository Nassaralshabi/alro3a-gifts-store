...
const sectionCopy: Record<string, SectionCopy> = {
  ...
  "custom-printing": { eyebrowAr: "من فكرتك إلى منتجك", eyebrowEn: "FROM IDEA TO PRINT", titleAr: "طباعة حسب الطلب", titleEn: "Made-to-order printing", subtitleAr: "اختاري المنتج وشاركي فكرتك لنرتب التفاصيل معك.", subtitleEn: "Choose a product and share your idea; we will handle the details." },
  "stickers-labels": { eyebrowAr: "تفاصيل صغيرة، أثر كبير", eyebrowEn: "SMALL DETAILS, BIG IMPACT", titleAr: "ستيكرات وليبلات", titleEn: "Stickers and labels", subtitleAr: "أكملي التغليف والهوية بلمسة مطبوعة مميزة.", subtitleEn: "Complete every package and identity with a printed finishing touch.", muted: true },
  "paper-bags": { eyebrowAr: "جاهزة للحمل والإهداء", eyebrowEn: "READY TO CARRY AND GIFT", titleAr: "أكياس ورقية", titleEn: "Paper bags", subtitleAr: "اختاري المقاس والتصميم المناسبين لطلبك.", subtitleEn: "Choose the right size and design for your order." },
...
export default function Home() {
  ...
  return <StoreShell logoUrl={homeContent?.logoImage}>
    ...
    <section className="bg-[#f8f6f0]"><div className="raed-container grid sm:grid-cols-3"><Benefit Icon={Truck} title={isArabic ? "توصيل لجميع الإمارات" : "UAE-wide delivery"} detail={isArabic ? "نجهز طلبك بعناية ونوصله" : "Prepared with care and delivered"} /><Benefit Icon={Sparkles} title={isArabic ? "تصاميم حسب الطلب" : "Made to order"} detail={isArabic ? "اختاري المقاس والتفاصيل" : "Choose the details and size"} /><Benefit Icon={Check} title={isArabic ? "جودة في كل تفصيلة" : "Care in every detail"} detail={isArabic ? "مطبوعات وتغليف بعناية" : "Thoughtful printing and wrapping"} /></div></section>
    {isLoading ? <section className="raed-container py-12"><div className="grid grid-cols-2 gap-4 sm:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-64 animate-pulse rounded-xl bg-[#e8f1f2]" />)}</div></section> : isError ? <section className="raed-container py-12"><div role="alert" className="rounded-xl border border-[#e6caca] bg-[#fff8f8] p-8 text-center"><p className="text-sm text-[#8b4b4b]">{isArabic ? "تعذر تحميل أقسام المنتجات. تحققي من الاتصال وحاولي مجددًا." : "Product sections could not be loaded. Check your connection and try again."}</p><Button onClick={() => refetch()} variant="outline" className="mt-4">{isArabic ? "إعادة المحاولة" : "Retry"}</Button></div></section> : <>
      <ProductSection eyebrow={isArabic ? "مختارات الروعة" : "AL RAWAA PICKS"} title={isArabic ? "اختيارات جاهزة لتُضاف لطلبك" : "Ready-to-request picks"} subtitle={isArabic ? "منتجات مختارة لتبدأي منها وتخصصيها حسب طلبك." : "Selected products to start with and customize for your order."} products={homeCatalog?.featured || []} viewAllHref="/shop" viewAllLabel={isArabic ? "عرض الكل" : "View all"} />
      ...
      <ProductSection eyebrow={isArabic ? "بكجات وعروض" : "GIFT PACKAGES"} title={isArabic ? "بكجات جاهزة لتبدأي منها" : "Gift packages to start with"} subtitle={isArabic ? "نماذج متاحة من الكتالوج يمكن تخصيص تفاصيلها حسب المناسبة والكمية." : "Available catalogue packages that can be tailored for your occasion and quantity."} products={homeCatalog?.bundles || []} viewAllHref="/shop?category=boxes-packaging" viewAllLabel={isArabic ? "عرض البوكسات" : "View boxes"} isMuted />
      <section className="raed-section-muted"><div className="raed-container py-11 sm:py-14"><div className="grid overflow-hidden rounded-2xl bg-[#17323b] text-white md:grid-cols-[1.1fr_.9fr]"><div className="p-7 sm:p-10"><p className="text-xs font-black uppercase tracking-[.18em] text-[#f2bd66]">{isArabic ? "هدية بطابعك" : "A GIFT, YOUR WAY"}</p><h2 className="mt-3 font-display text-3xl leading-tight sm:text-4xl">{isArabic ? "اجعلي فكرتك هدية ملموسة" : "Turn your idea into a tangible gift"}</h2><p className="mt-4 max-w-lg text-sm leading-7 text-white/75">{isArabic ? "اختاري المنتج، ثم أرسلي تفاصيل المقاس أو المناسبة أو الكمية لنرتّب طلبك معك." : "Choose a product, then share the size, occasion, or quantity so we can prepare your request."}</p><div className="mt-7 flex flex-wrap gap-3"><Button asChild className="bg-[#f2bd66] text-[#17323b] hover:bg-[#ffd282]"><a href={contact.whatsappUrl} target="_blank" rel="noreferrer">{isArabic ? "ابدئي طلبًا مخصصًا" : "Start a custom request"}</a></Button><Button asChild variant="outline" className="border-white/35 bg-transparent text-white hover:bg-white hover:text-[#17323b]"><Link href="/shop?category=boxes-packaging">{isArabic ? "استكشفي البوكسات" : "Explore boxes"}</Link></Button></div></div><div className="relative min-h-64"><img src={promoImage} alt={isArabic ? "تغليف وهدايا من مطبعة الروعة" : "Al Rawaa packaging and gifts"} width={960} height={640} loading="lazy" decoding="async" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-l from-transparent to-[#17323b]/40" /></div></div></div></section>
      {catalogSections.slice(2).map(section => <ProductSection key={section.slug} eyebrow={isArabic ? section.copy.eyebrowAr : section.copy.eyebrowEn} title={isArabic ? section.copy.titleAr : section.copy.titleEn} subtitle={isArabic ? section.copy.subtitleAr : section.copy.subtitleEn} products={section.products} viewAllHref={`/shop?category=${section.slug}`} viewAllLabel={isArabic ? "عرض الكل" : "View all"} isMuted={section.copy.muted} />)}
    </>}
    <section className="raed-section-muted"><div className="raed-container grid gap-5 py-11 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="raed-kicker">{isArabic ? "هل لديك فكرة مختلفة؟" : "HAVE A DIFFERENT IDEA?"}</p><h2 className="mt-2 font-display text-2xl text-[#17323b] sm:text-3xl">{isArabic ? "شاركي تفاصيل مناسبتك لنساعدك في اختيار الأنسب" : "Share your occasion details and we will help you choose"}</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-[#688086]">{isArabic ? "نساعدك في اختيار نوع المنتج والمقاس والتغليف حسب الكمية والمناسبة." : "We can help choose the product, size, and wrapping for your quantity and occasion."}</p></div><Button asChild className="h-11 rounded-md bg-[#16717d] px-5 hover:bg-[#105d67]"><a href={contact.whatsappUrl} target="_blank" rel="noreferrer">{isArabic ? "تواصلي معنا" : "Contact us"}<ArrowUpLeft className="h-4 w-4" /></a></Button></div></section>
  </StoreShell>;
}
