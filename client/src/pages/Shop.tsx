...
import { getCatalogPageSize, shouldAutoLoadMobileCatalogPage } from "@/lib/mobileInfiniteBrowse";
...
export default function Shop() {
  ...
  const mobileNextPageRequestRef = useRef(false);
  ...
  const catalogInput = useMemo(() => ({ categorySlug: category === "all" ? undefined : category, query: query || undefined, priceOrder, limit: getCatalogPageSize(isMobile) }), [category, query, priceOrder, isMobile]);
  ...
  useEffect(() => {
    if (!shouldAutoLoadMobileCatalogPage({ isMobile, hasNextPage: Boolean(catalog.hasNextPage), isFetchingNextPage: catalog.isFetchingNextPage })) {
      mobileNextPageRequestRef.current = false;
      return;
    }
    const target = loadMoreRef.current;
    if (!target || typeof IntersectionObserver === "undefined") return;
    const observer = new IntersectionObserver(entries => {
      if (!entries.some(entry => entry.isIntersecting) || mobileNextPageRequestRef.current) return;
      mobileNextPageRequestRef.current = true;
      void catalog.fetchNextPage().finally(() => { mobileNextPageRequestRef.current = false; });
