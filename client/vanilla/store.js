...
document.addEventListener("change", event => { if (event.target.id === "order") { state.priceOrder = event.target.value; renderShop(); } if (event.target.id === "price-range") { state.priceRange = event.target.value; renderShop(); } if (event.target.id === "occasion") { state.occasion = event.target.value; renderShop(); } });
