...
for (const device of [{ name: "desktop", viewport: { width: 1280, height: 720 }, isMobile: false }, { name: "mobile", viewport: { width: 375, height: 812 }, isMobile: true }]) {
  ...
  const heroResources = await page.evaluate(() => performance.getEntriesByType("resource").map(entry => ({ name: entry.name, transferSize: entry.transferSize })).filter(entry => /banner|hero|national-day/i.test(entry.name)));
  reports.push({ device: device.name, waitMs: 2800, activeBefore, activeAfter, automaticallyAdvanced: activeBefore !== activeAfter, heroResources, consoleErrors: errors });
  await context.close();
}
