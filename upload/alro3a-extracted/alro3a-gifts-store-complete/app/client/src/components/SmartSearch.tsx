...
import { COMMON_SEARCHES } from "@/lib/commonSearches";
...
import { ArrowUpLeft, Boxes, FolderSearch2, Gift, LayoutPanelTop, LoaderCircle, Package, PackageSearch, Paintbrush, Search, Sparkles, Stamp, Tag, TrendingUp, Trophy, type LucideIcon } from "lucide-react";
...
export default function SmartSearch({ autoFocus = false, onNavigate, className = "" }: { autoFocus?: boolean; onNavigate?: () => void; className?: string }) {
  ...
  function navigateToCommonSearch(href: string) {
    setLocation(href);
    setIsOpen(false);
    onNavigate?.();
  }
  ...
  const showCommonSearches = !hasRawQuery;
